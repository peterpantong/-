...
| 아난티 남해 (경남 남해) | `data/ananti_namhae.json` | **18홀 완료** — par/HDCP/티거리/체크포인트/공략 (도면 이미지만 미포함) |
...
**아난티 남해**는 18홀이 다 채워져 있습니다 (`reference/` 자료에서 추출·검증).
**부곡CC**는 아직 파 / 핸디캡 / 티별 거리가 비어 있습니다(null) — 공식 홈페이지가
이 작업 환경의 네트워크 정책에서 차단되어 가져올 수 없었고, 확인되지 않은 거리를
지어내지 않았습니다.

비어 있는 상태로 빌드해도 **라운드 중 직접 적어 넣는 백지 가이드**로 쓸 수 있고,
아래 값을 채우면 모든 표·도면·플랜이 실제 값으로 채워집니다.
숫자를 옮길 때는 [reference/제작_노트.md](reference/제작_노트.md) 의 **검증 5가지**를 돌리세요.
...
`reference/` 에 아난티 남해 완성본(마크다운 · 22쪽 PDF)과 제작 절차 문서가 들어 있습니다.
**[reference/제작_노트.md](reference/제작_노트.md)** 에 다음 코스를 만들 때 쓸 것들을 정리했습니다 —
더 나은 홀 데이터 스키마, 참조본 홀 페이지 레이아웃, 숫자를 옮길 때 쓰는 검증 5가지,
검색 요약을 믿으면 안 되는 이유, 이 작업 환경의 반입 경로.

아난티 남해 데이터에는 `title` / `marks`(체크포인트) / `play`(티샷·세컨드·그린) /
`caution` 이 18홀 전부 채워져 있습니다. **현재 렌더러는 이 필드를 아직 쓰지 않고**
보관만 합니다 — 다음 레이아웃 작업에서 쓰려고 남겨 둔 것입니다.
...
data/bugok_cc.json        부곡CC 코스 데이터 (미입력)
data/ananti_namhae.json   아난티 남해 코스 데이터 (18홀 완료)
data/sample_filled.json   레이아웃 확인용 예시 데이터 (실제 거리 아님)
images/                   골프장 제공 코스안내도 이미지
reference/                참고 자료 + 제작 노트 (다음 코스 만들 때)
```
